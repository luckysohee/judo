...
async function processPlaceNamesToSQL() {
  try {
    ...
    // SQL 파일 생성 (place_id 포함)
    generateSQLWithPlaceId(results);
...
// SQL 파일 생성 (place_id 포함)
function generateSQLWithPlaceId(places) {
  let sql = `-- 큐레이터 장소 데이터 import (상호명 기반) - curator_id: ${CURATOR_ID}\n`;
  sql += '-- 상호명으로 카카오지도 검색 성공한 장소들만 포함\n';
  sql += '-- place_id 포함하여 카카오 지도 정보 바로 표시 가능\n\n';

  places.forEach((place, index) => {
    sql += `-- ${index + 1}. ${place.place_name} (카카오 ID: ${place.place_id})\n`;
    sql += `INSERT INTO places (name, address, lat, lng, place_id, created_at) \n`;
    sql += `SELECT '${place.place_name}', '${place.address}', ${place.y}, ${place.x}, '${place.place_id}', NOW()\n`;
  ...
  fs.writeFileSync('./curator_places_with_place_id.sql', sql);
  console.log('\n📄 SQL 파일 생성: curator_places_with_place_id.sql');
}
