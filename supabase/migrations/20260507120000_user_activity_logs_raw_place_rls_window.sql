-- user_activity_logs: places 매칭 실패 시에도 raw로 로그 유지
-- RLS SELECT: 본인 + 맞픽 행 허용
-- get_mutual_checkins: 최근 12시간, limit 상한 30 — 이 함수는 이 마이그레이션에만 두고 061400에는 없음(OVERLOAD 충돌 방지)
...
-- 반환 타입(OUT) 바뀌면 OR REPLACE 불가 → 스키마·인자 문자열 기준 ROUTINE 전부 DROP
DO $$
DECLARE
  r RECORD;
BEGIN
