-- 예전 get_places_in_bounds 가 public.places.place_url 을 SELECT 해 42703 → /api/places-in-bounds 500.
-- 마이그레이션 히스토리와 무관하게 함수 본문을 최신 정의로 다시 덮어씀 (cand 에서 place_url 은 리터럴 '').
