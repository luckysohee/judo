-- 코스 검색: 큐레이터 별명(name)·핸들(slug/username)도 매칭 (profiles 외에 curators 조인)
...
BEGIN
  ...
  WITH matched AS (
    ...
    LEFT JOIN public.curators cu ON cu.user_id = cc.curator_id
...
COMMENT ON FUNCTION public.search_public_curator_courses(text, integer, integer) IS
  '홈 코스 discovery 검색 — 제목·지역·태그 + profiles/curators 별명·핸들. limit+1로 has_more.';
