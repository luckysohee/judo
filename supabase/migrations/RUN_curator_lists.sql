-- 맛집첩(curator_lists) 부트스트랩 — Supabase Dashboard → SQL Editor 에서 한 번에 실행
-- (= 20260719120000_curator_lists.sql + 20260719130000_curator_list_places_image_url.sql)
...
-- 확인
SELECT
  to_regclass('public.curator_lists') AS curator_lists,
  to_regclass('public.curator_list_places') AS curator_list_places,
  EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = 'curator_list_places'
