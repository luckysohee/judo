-- 잔 아카이브 「내 스타일 분석」: curator_places + 잔 코스 장소·테마 태그 합산 (취향만, 큐레이터 단위)
-- one_line_top / followers 는 장소 픽 기준 유지
  ...
  style_pick_rows AS (
    ...
    tg AS (
      ...
      FROM (
        ...
        SELECT DISTINCT
          ('ct-' || cc.id::text || '-' || trim(t.x)) AS cp_id,
          trim(t.x) AS label
        FROM curator_courses cc
...
COMMENT ON FUNCTION public.studio_archive_extended_insights(uuid) IS
  '잔 아카이브 인사이트. style 은 curator_places + 잔 코스 장소·theme_tags 합산(취향). one_line/followers 는 장소 픽 기준.';
