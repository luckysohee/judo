-- 코스 픽: 조회 RPC + 동시 요청·장소 복사 재시도 안정화 (409 Conflict 방지)

CREATE OR REPLACE FUNCTION public.get_my_imported_course_snapshot_id(p_source_course_id uuid)
RETURNS uuid
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  ...
  WHERE cc.curator_id = auth.uid()
    AND cc.imported_from_course_id = p_source_course_id
  LIMIT 1;
$$;

REVOKE ALL ON FUNCTION public.get_my_imported_course_snapshot_id(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_my_imported_course_snapshot_id(uuid) TO authenticated;
...
BEGIN
  ...
  IF v_src.status <> 'published' OR NOT v_src.is_public THEN
    RAISE EXCEPTION '공개된 코스만 픽할 수 있습니다.' USING ERRCODE = 'P0001';
  END IF;

  IF v_src.curator_id = v_user THEN
    RAISE EXCEPTION '내가 만든 코스는 픽할 수 없습니다.' USING ERRCODE = 'P0001';
  END IF;
  ...
  IF v_step_count < 1 THEN
    RAISE EXCEPTION '장소가 없는 코스는 픽할 수 없습니다.' USING ERRCODE = 'P0001';
  END IF;
