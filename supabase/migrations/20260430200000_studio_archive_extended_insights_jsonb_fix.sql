-- alcohol_types / moods / tags 가 jsonb 또는 text[] 인 DB 모두 지원
-- COALESCE(cp.alcohol_types, '[]'::jsonb) 는 컬럼이 text[] 이면 타입 불일치 → RPC 500
-- unnest 전용: jsonb | text[] NULL → text[] 로 정규화 (cp_str_arr_for_unnest)
-- curator_places / user_follows 의 curator_id 가 text·uuid 혼재 시 btrim(…::text) 로 비교 (42883)
