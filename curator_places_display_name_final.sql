...
-- 📞 전화번호: 정보없음
-- 📍 주소: 서울 광진구 중곡동 639-6
-- 🏷️ 카테고리: 서비스,산업 > 전문대행 > 공간대여 > 스터디카페,스터디룸
INSERT INTO places (name, address, lat, lng, created_at) 
...
-- 📞 전화번호: 정보없음
-- 📍 주소: 서울 용산구 용산동2가 1-63
-- 🏷️ 카테고리: 문화,예술 > 사진 > 사진관,포토스튜디오 > 즉석사진
INSERT INTO places (name, address, lat, lng, created_at) 
...
-- 📞 전화번호: 0507-1498-7007
-- 📍 주소: 서울 강남구 역삼동 812-13
-- 🏷️ 카테고리: 음식점 > 양식 > 피자
INSERT INTO places (name, address, lat, lng, created_at) 
SELECT '퍼그 피자하우스 강남점', '서울 강남구 역삼동 812-13', 37.502744, 127.027429, NOW()
WHERE NOT EXISTS (SELECT 1 FROM places WHERE name = '퍼그 피자하우스 강남점');

-- 18. 구이천국 (카카오 ID: 1978639744)
-- 📞 전화번호: 02-825-6225
-- 📍 주소: 서울 동작구 노량진동 15-27
-- 🏷️ 카테고리: 음식점 > 한식 > 해물,생선 > 조개
INSERT INTO places (name, address, lat, lng, created_at) 
SELECT '구이천국', '서울 동작구 노량진동 15-27', 37.514236, 126.942478, NOW()
WHERE NOT EXISTS (SELECT 1 FROM places WHERE name = '구이천국');

-- 19. 구워삶다 (카카오 ID: 27583385)
-- 📞 전화번호: 정보없음
-- 📍 주소: 서울 마포구 서교동 397-10
-- 🏷️ 카테고리: 음식점 > 한식 > 육류,고기
INSERT INTO places (name, address, lat, lng, created_at) 
SELECT '구워삶다', '서울 마포구 서교동 397-10', 37.55011, 126.91856, NOW()
...
-- 🏷️ 카테고리: 음식점 > 술집
INSERT INTO places (name, address, lat, lng, created_at) 
SELECT '술잔코드', '서울 마포구 서교동 396-4', 37.55083, 126.912853, NOW()
WHERE NOT EXISTS (SELECT 1 FROM places WHERE name = '술잔코드');

-- 21. 마초나초 (카카오 ID: 6504788)
-- 📞 전화번호: 0502-5552-2529
-- 📍 주소: 서울 마포구 망원동 418-17
-- 🏷️ 카테고리: 음식점 > 양식
INSERT INTO places (name, address, lat, lng, created_at) 
SELECT '마초나초', '서울 마포구 망원동 418-17', 37.554046, 126.898694, NOW()
WHERE NOT EXISTS (SELECT 1 FROM places WHERE name = '마초나초');

-- 22. 폴바셋 한남커피스테이션로스트웍스 (카카오 ID: 26852037)
-- 📞 전화번호: 02-792-8310
-- 📍 주소: 서울 용산구 한남동 257-13
-- 🏷️ 카테고리: 음식점 > 카페 > 커피전문점 > 폴바셋
INSERT INTO places (name, address, lat, lng, created_at) 
SELECT '폴바셋 한남커피스테이션로스트웍스', '서울 용산구 한남동 257-13', 37.534093, 127.008086, NOW()
WHERE NOT EXISTS (SELECT 1 FROM places WHERE name = '폴바셋 한남커피스테이션로스트웍스');

-- 23. 트리니티와인앤컬처 (카카오 ID: 1779163964)
-- 📞 전화번호: 02-790-1302
-- 📍 주소: 서울 용산구 한남동 274-10
-- 🏷️ 카테고리: 서비스,산업 > 식품 > 음료,주류제조 > 주류도매,주류유통
INSERT INTO places (name, address, lat, lng, created_at) 
SELECT '트리니티와인앤컬처', '서울 용산구 한남동 274-10', 37.533138, 127.008949, NOW()
WHERE NOT EXISTS (SELECT 1 FROM places WHERE name = '트리니티와인앤컬처');

-- 24. 도연회포차 (카카오 ID: 557142812)
-- 📞 전화번호: 02-2272-2274
-- 📍 주소: 서울 종로구 관수동 4-1
-- 🏷️ 카테고리: 음식점 > 한식 > 해물,생선 > 회
INSERT INTO places (name, address, lat, lng, created_at) 
SELECT '도연회포차', '서울 종로구 관수동 4-1', 37.570347, 126.991357, NOW()
...
-- 🏷️ 카테고리: 음식점 > 카페
INSERT INTO places (name, address, lat, lng, created_at) 
SELECT '그린마일커피 북촌점', '서울 종로구 가회동 11-49', 37.580714, 126.985099, NOW()
WHERE NOT EXISTS (SELECT 1 FROM places WHERE name = '그린마일커피 북촌점');

-- 26. 쏘플파티룸 낙산파라도르 루프탑 (카카오 ID: 1759990263)
-- 📞 전화번호: 1544-3523
-- 📍 주소: 서울 성북구 삼선동1가 279-1
-- 🏷️ 카테고리: 서비스,산업 > 전문대행 > 공간대여
INSERT INTO places (name, address, lat, lng, created_at) 
SELECT '쏘플파티룸 낙산파라도르 루프탑', '서울 성북구 삼선동1가 279-1', 37.581965, 127.002617, NOW()
WHERE NOT EXISTS (SELECT 1 FROM places WHERE name = '쏘플파티룸 낙산파라도르 루프탑');

-- 27. 판자집 (카카오 ID: 8121543)
-- 📞 전화번호: 정보없음
-- 📍 주소: 서울 서대문구 창천동 29-18
-- 🏷️ 카테고리: 음식점 > 술집 > 호프,요리주점
INSERT INTO places (name, address, lat, lng, created_at) 
SELECT '판자집', '서울 서대문구 창천동 29-18', 37.556268, 126.942762, NOW()
...
-- 🏷️ 카테고리: 여행 > 숙박 > 여관,모텔
INSERT INTO places (name, address, lat, lng, created_at) 
SELECT '헤르츠호텔', '서울 종로구 낙원동 108-1', 37.572826, 126.990956, NOW()
...
-- 🏷️ 카테고리: 음식점 > 술집 > 호프,요리주점
INSERT INTO places (name, address, lat, lng, created_at) 
SELECT '영동호프&식당', '서울 중구 수표동 11-3', 37.567317, 126.991001, NOW()
