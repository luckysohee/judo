-- supabase/migrations/20260603140000_get_places_in_bounds_curator_priority.sql 과 동기

-- 홈 지도 bbox: 큐레이터 추천 우선 정렬 + 줌 인용 limit 상한 200
-- menu_reason 미존재 DB(42703) 호환 — JSON에는 빈 문자열
...
BEGIN
  ...
  RETURN (
    ...
    cand AS (
      ...
      LEFT JOIN curator_counts cc ON cc.place_id = p.id
...
COMMENT ON FUNCTION public.get_places_in_bounds(double precision, double precision, double precision, double precision, integer) IS
  '지도 bbox: 큐레이터 추천 수 우선 places 상한(최대 200) + 비아카이브 curator_places. service_role 전용.';
