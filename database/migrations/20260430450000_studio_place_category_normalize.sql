-- 잔 올리기 표준 업종 정규화: Kakao `category_name` 등 → 한글 버킷 (JS `normalizeStudioPlaceCategory` 와 동일 의도)
-- + 아카이브 `style.categories` 집계에 `studio_normalize_place_category` 적용

CREATE OR REPLACE FUNCTION public.cp_str_arr_for_unnest(a jsonb)
RETURNS text[]
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN a IS NULL OR jsonb_typeof(a) <> 'array' THEN ARRAY[]::text[]
    ELSE COALESCE(ARRAY(SELECT jsonb_array_elements_text(a)), ARRAY[]::text[])
  END;
$$;

CREATE OR REPLACE FUNCTION public.cp_str_arr_for_unnest(a text[])
RETURNS text[]
LANGUAGE sql
IMMUTABLE
AS $$
...
COMMENT ON FUNCTION public.cp_str_arr_for_unnest(jsonb) IS
  'studio_archive_extended_insights: jsonb 배열 → text[]';
COMMENT ON FUNCTION public.cp_str_arr_for_unnest(text[]) IS
  'studio_archive_extended_insights: text[] 그대로';

CREATE OR REPLACE FUNCTION public.studio_normalize_place_category(p_raw text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN p_raw IS NULL OR btrim(p_raw) = '' THEN '미분류'::text
    WHEN btrim(p_raw) IN (
      '한식', '중식', '일식', '양식', '육류', '해산물', '디저트', '술집·바', '미분류'
    ) THEN btrim(p_raw)
    WHEN lower(btrim(p_raw)) ~ '(중식|짜장|짬뽕)' THEN '중식'::text
    WHEN lower(btrim(p_raw)) ~ '(일식|돈까스|돈가스|라멘|라면|우동|일본|오마카세|스시|초밥)' THEN '일식'::text
    WHEN lower(btrim(p_raw)) ~ '(양식|파스타|피자|스테이크|프렌치|이탈리아|브런치|버거)' THEN '양식'::text
    WHEN lower(btrim(p_raw)) ~ '(해산물|횟집|회집|생선회|생선|조개|새우|사시미|참치|해물|오징어|게장|물회|수산|회전)' THEN '해산물'::text
    WHEN lower(btrim(p_raw)) ~ '(디저트|카페|베이커리|케이크|빵|커피|아이스크림|도넛)' THEN '디저트'::text
    WHEN lower(btrim(p_raw)) ~ '(술집|와인바|와인|이자카야|호프|주점|펍|포장마차|포차|루프탑|야장|칵테일|클럽|라운지|노포|실내포장|야외포장|테라스|요리주점|맥주|소주)' THEN '술집·바'::text
    WHEN lower(btrim(p_raw)) ~ '(돼지|소고기|고기|구이|삼겹|갈비|육류|닭|치킨|족발|보쌈|곱창|막창)' THEN '육류'::text
    WHEN lower(btrim(p_raw)) ~ '(한식|국밥|탕|찌개|순대|감자탕|설렁탕|해장|국수|냉면|비빔밥|순두부|곰탕|백반|죽|한우|밥집)' THEN '한식'::text
    WHEN lower(btrim(p_raw)) ~ '(슈퍼|마트|편의점|이마트)' THEN '미분류'::text
    WHEN lower(btrim(p_raw)) ~ '순대' THEN '한식'::text
    ELSE '미분류'::text
  END;
$$;

COMMENT ON FUNCTION public.studio_normalize_place_category(text) IS
  'Kakao category_name 등 → Studio 잔 올리기 표준 업종 (places.category 저장·아카이브 집계와 맞춤)';
...
BEGIN
  ...
  RETURN (
    ...
    cat AS (
      SELECT public.studio_normalize_place_category(trim(p.category)) AS label,
        COUNT(*)::bigint AS cnt
      FROM curator_places cp
      ...
      WHERE btrim(cp.curator_id::text) IN (SELECT btrim(cid::text) FROM my_curator_ids)
        ...
        AND public.studio_normalize_place_category(trim(p.category)) <> ALL (
          ARRAY['기타', '미분류']::text[]
        )
      GROUP BY 1
    ),
