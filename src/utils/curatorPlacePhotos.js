...
/**
 * 코스 커버 전용: `curator-place-photos` `{userId}/course-covers/...` (DB 행 없음)
 * Storage RLS: 큐레이터는 `{auth.uid()}/...` 하위 경로에 INSERT 가능
 * @returns {Promise<string>} 공개 URL
 */
export async function uploadCuratorCourseCoverFile(file, curatorUserId) {
  if (!file || !curatorUserId) {
  ...
  const rawName = (prepared.name || "cover").toLowerCase();
  const ext = rawName.includes(".")
    ? rawName.split(".").pop()
    : (prepared.type || "").includes("png")
      ? "png"
      : (prepared.type || "").includes("webp")
        ...
        : (prepared.type || "").includes("gif")
          ...
          : "jpg";
  const safeExt = ["jpg", "jpeg", "png", "webp", "gif"].includes(ext) ? ext : "jpg";
  const path = `${curatorUserId}/course-covers/cover-${Date.now()}-${Math.random()
    .toString(36)
    .slice(2, 9)}.${safeExt}`;
  ...
  if (upErr) {
    const extra =
      upErr.message?.includes("Bucket not found") || upErr.message?.includes("not found")
        ? " Supabase에서 버킷 curator-place-photos(Public)와 Storage 정책을 확인하세요."
        : upErr.message?.includes("new row violates") || upErr.message?.includes("policy")
          ? " Storage RLS: 큐레이터 계정으로 로그인했는지, 경로가 '{본인 user id}/course-covers/...' 인지 확인하세요."
          : "";
    throw new Error((upErr.message || "커버 업로드 실패") + extra);
  }
