...
const COURSE_AREA_HARD_EXCLUDE = {
  ...
  // 종로 bbox(3.2km)에 종로 축 밖 권역이 대거 끼던 문제.
  // 무교동·다동(남대문로9길·남대문로60 등 북쪽)은 시청/청계천 옆이라 유지하고,
  // 남대문시장·회현(남) / 충정로·서대문(서) / 만리·중림(서남, 서울역) / 용산(남)만 끊는다.
  종로: [
    // 용산구(숙대입구·남영) 권역
    /용산구/,
    /남영/,
    ...
    // 중구 남동(신당·약수)
    /신당|다산|약수/,
    // 남대문시장·회현(중구 남) — 단, 무교동·다동의 '남대문로N길/번지'는 제외 안 함
    /회현/,
    /남대문로[2-6]가|남대문시장/,
    // 충정로·서대문·신촌 경계(서)
    /충정로/,
    ...
    /창천동/,
    /영천|독립문/,
    // 만리·중림(서남, 서울역 권역) — 상호 '만리향' 오매칭 피하려 위치 토큰만
    /만리재|만리동|만리시장/,
    /중림/,
    /봉래|의주로|서소문|순화동/,
  ],
...
const COURSE_AREA_CORE_RADIUS_KM = {
  을지로: 1.2,
  종로: 1.8, // 광화문~종로6가 E-W 축. 남서(남대문·충정로·만리)는 하드 제외로 별도 차단
};
