...
import { parseNaturalQuery, scorePlace, generateAlternativeSuggestions, SEARCH_EXAMPLES, POPULAR_TAGS } from "../../utils/naturalLanguageParser";
...
export default function Home() {
  ...
  const enrichPlaceWithKakaoInfo = async (place) => {
    ...
    try {
      console.log('🔍 카카오 장소 정보 조회:', place.name);
      
      const response = await fetch(`https://dapi.kakao.com/v2/local/search/keyword.json?query=${encodeURIComponent(place.name)}&size=1`, {
        headers: {
  ...
  const [showAll, setShowAll] = useState(true); // 모든 큐레이터 표시 여부

const [aiSummary, setAiSummary] = useState("");
const [aiReasons, setAiReasons] = useState([]);
const [aiRecommendedIds, setAiRecommendedIds] = useState([]);
const [aiRecommendedPlaces, setAiRecommendedPlaces] = useState([]);
...
const [showUserCard, setShowUserCard] = useState(false); // UserCard 표시 상태
      ...
      try {
        ...
        const enrichedPlaces = await Promise.all(
          data.map(async (curatorPlace) => {
            ...
            // 카카오 정보로 장소 확장
            const enrichedPlace = await enrichPlaceWithKakaoInfo(place);
  ...
  const displayedPlaces = useMemo(() => {
    if (!query || !query.trim()) return filteredByCuratorPlaces;
...
const topReasonMap = useMemo(() => {
  ...
  // 룰 기반 검색 함수
  const handleRuleBasedSearch = (query) => {
    if (!query.trim()) {
    ...
    console.log('🔍 룰 기반 검색 시작:', query);
    console.log('📊 dbPlaces 데이터:', dbPlaces?.length || 0, '개');

    // 1. 자연어 파싱
    const parsedQuery = parseNaturalQuery(query);
    console.log('🧩 파싱 결과:', parsedQuery);
    ...
    // 2. 모든 장소에 대해 점수 계산 (dbPlaces가 없으면 더미 데이터 사용)
    const placesToScore = dbPlaces && dbPlaces.length > 0 ? dbPlaces : dummyPlaces;
    console.log('📊 점수 계산할 장소:', placesToScore.length, '개');
    
    const scoredPlaces = placesToScore.map(place => {
      const scored = scorePlace(place, parsedQuery);
      return {
        ...place,
        aiScore: scored.score,
        recommendation: scored.reasons.join(', '),
    ...
    // 3. 점수순 정렬 및 필터링
    const filteredResults = scoredPlaces
      .filter(place => place.aiScore > 0)
      .sort((a, b) => b.aiScore - a.aiScore)
      .slice(0, 10);

    console.log('📊 검색 결과:', filteredResults.length, '개');

    if (filteredResults.length === 0) {
      // 4. 결과 없을 때 대안 제안
      const alternatives = generateAlternativeSuggestions(parsedQuery, dbPlaces || []);
      console.log('💡 대안 제안:', alternatives);
      
      if (alternatives.length > 0) {
        showToast(alternatives[0].message, 'info', 3000);
        setAiRecommendedPlaces(alternatives[0].places);
      } else {
        showToast('검색 결과가 없습니다. 다른 키워드로 시도해보세요!', 'warning', 3000);
      }
    } else {
      // 5. 결과 있을 때 추천으로 표시
      setAiRecommendedPlaces(filteredResults);
      setAiSheetOpen(true);
      
      // aiRecommendedIds도 설정하여 displayedPlaces가 동작하게 함
      const recommendedIds = filteredResults.map(place => place.id);
      ...
      console.log('📝 검색 로그 저장:', {
        ...
        top_result: filteredResults[0]?.name
      });
    }
  ...
  const handleSearchSubmit = async (value) => {
    const nextQuery = value && value.trim ? value.trim() : '';
    ...
    try {
      ...
      // 룰 기반 검색 실행
      handleRuleBasedSearch(nextQuery);

    } catch (error) {
      console.error('검색 실패:', error);
      setAiError('검색에 실패했습니다. 다시 시도해주세요.');
      setIsAiSearching(false);
    }
  };
    ...
    <>
      ...
      <main style={styles.mainContainer}>
        ...
        <MapView
          ...
          userFolders={folders} // 사용자 폴더 정보 전달
          ...
          userSavedPlaces={savedMap} // 사용자 저장 장소 정보 전달
        />
