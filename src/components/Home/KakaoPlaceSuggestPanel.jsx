...
export default function KakaoPlaceSuggestPanel({
  ...
  const primarySearchRow =
    trimmedQuery && typeof onSubmitQuery === "function" ? (
      <button
        type="button"
        style={s.primarySearchRow}
        onClick={() => onSubmitQuery(trimmedQuery)}
      >
        <span style={s.rowIcon} aria-hidden>
          🔍
        </span>
        <span style={{ ...s.rowMain, flex: 1, minWidth: 0, whiteSpace: "normal" }}>
          {trimmedQuery}
        </span>
      </button>
    ) : null;
